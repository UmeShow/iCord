import { Client, GatewayIntentBits, Message, Partials, ChannelType, Interaction, CacheType, CommandInteraction, ChatInputCommandInteraction, MessageFlags, Events, SlashCommandBuilder } from 'discord.js';
import { EndBehaviorType, VoiceConnection, VoiceConnectionStatus, entersState, getVoiceConnection, joinVoiceChannel } from '@discordjs/voice';
import prism from 'prism-media';
import { ICharacter } from '../database/models/Character';
import { generateResponse, PromptPart } from '../ai/grok';
import { ConversationService } from '../database/ConversationService';
import { db } from '../database/firebase';
import { IUser } from '../database/models/User';
import { commands as gameCommands, Command } from './commands';
import { botRegistry } from './BotRegistry';

export class BotInstance {
    private static activeBotChatChannels: Set<string> = new Set();

    private client: Client;
    private character: ICharacter;
    private isRunning: boolean = false;
    private isAutoReply: boolean = false;
    // Timestamp for the last memory reset (triggered by /wack)
    private lastResetTime: number = 0;
    // Track latest wack token to avoid re-applying.
    private lastWackToken: string | undefined;
    // Simple in-memory rate limiting: userId -> timestamp
    private lastMessageTime: Map<string, number> = new Map();

    private voiceConnection: VoiceConnection | null = null;
    private voiceGuildId: string | null = null;
    private voiceTextChannelId: string | null = null;
    private voiceActiveSubscriptions: Set<string> = new Set();
    ...
    public get characterNickname(): string | undefined {
        return this.character.nickname;
    }

    public get botUserId(): string | undefined {
        return this.client.user?.id;
    }

    private getErrorReply(): string {
    ...
    private async fetchAsBase64(url: string, maxBytes: number): Promise<{ base64: string; mimeType: string } | null> {
    ...
    private isImageAttachment(att: { contentType?: string | null; name?: string | null }): boolean {
    ...
    private isAudioAttachment(att: { contentType?: string | null; name?: string | null }): boolean {
        const ct = (att.contentType || '').toLowerCase();
    ...
    private async transcribeAudioFromAttachment(url: string, fileName?: string | null, contentType?: string | null): Promise<string> {
        ...
        const transcript = await generateResponse(parts, [], undefined, {
            temperature: 0.2,
            topP: 0.9,
            topK: 20,
    ...
    private async registerCommands() {
        ...
        const gameCommandData = gameCommands.map(cmd => cmd.data.toJSON());
    ...
    private async handleInteraction(interaction: Interaction<CacheType>) {
        ...
        if (commandName === 'wack') {
            ...
            await interaction.reply({ content: "Auto-reply mode activated! I will now respond to all messages in this channel.", flags: MessageFlags.Ephemeral });
        } else if (commandName === 'deactivate') {
            this.isAutoReply = false;
            await interaction.reply({ content: "Auto-reply mode deactivated. I will only respond when mentioned.", flags: MessageFlags.Ephemeral });
            ...
            const command = gameCommands.find(c => c.data.name === commandName);
                ...
                try {
                    ...
                    try {
                        if (interaction.replied || interaction.deferred) {
                            await interaction.followUp({ content: this.getErrorReply(), flags: MessageFlags.Ephemeral });
                        } else {
                            await interaction.reply({ content: this.getErrorReply(), flags: MessageFlags.Ephemeral });
                        }
                    } catch (replyError) {
    ...
    private async handleBotsChat(interaction: ChatInputCommandInteraction) {
        ...
        const otherBot = botRegistry.findByName(otherName, this.characterId);
        if (!otherBot) {
            const available = botRegistry.listDisplayNames();
            await interaction.reply({
    ...
    private async handleVcJoin(interaction: ChatInputCommandInteraction) {
        try {
            ...
            if (!voiceChannel) {
                await interaction.reply({ content: 'まずVCに入ってから実行してね。', flags: MessageFlags.Ephemeral });
                return;
            }
            ...
            if (this.voiceConnection && this.voiceGuildId === guild.id) {
                ...
                await interaction.reply({ content: `VC接続中。文字起こしの投稿先をこのチャンネルに変更しました。`, flags: MessageFlags.Ephemeral });
                return;
            }
            ...
            try {
                ...
                await this.leaveVoice();
                throw new Error(`Failed to connect to voice channel: ${(stateError as any).message}`);
            }
            ...
            console.error(`[Bot ${this.character.name}] Error stack:`, (e as any)?.stack);
            try {
                if (interaction.replied || interaction.deferred) {
                    await interaction.followUp({ content: `接続に失敗しました: ${(e as any)?.message || this.getErrorReply()}`, flags: MessageFlags.Ephemeral });
                } else {
                    await interaction.reply({ content: `接続に失敗しました: ${(e as any)?.message || this.getErrorReply()}`, flags: MessageFlags.Ephemeral });
                }
            } catch {}
        }
    }

    private async handleVcLeave(interaction: ChatInputCommandInteraction) {
        try {
            console.log(`[Bot ${this.character.name}] User requested VCLeave`);
            await this.leaveVoice();
            await interaction.reply({ content: 'VCから退出したよ。', flags: MessageFlags.Ephemeral });
        } catch (e) {
            console.error(`[Bot ${this.character.name}] vcleave error:`, e);
            try {
                await interaction.reply({ content: 'VCから退出中にエラーが発生しました。', flags: MessageFlags.Ephemeral });
            } catch {}
        }
    }

    private async leaveVoice() {
        try {
    ...
    private voiceConversationHistory: Map<string, { role: 'user' | 'model'; parts: string }[]> = new Map();

    private startVoiceReceiver(connection: VoiceConnection) {
        ...
        receiver.speaking.on('start', (userId: string) => {
            ...
            opusStream
                ...
                .pipe(decoder)
                .on('data', (data: Buffer) => {
                    if (totalBytes >= MAX_PCM_BYTES) return;
                    chunks.push(data);
                ...
                .on('end', async () => {
                    ...
                    try {
                        ...
                        const transcript = await generateResponse(
                            [
                            ...
                            [],
                            undefined,
                            { temperature: 0.2, topP: 0.9, topK: 20, maxOutputTokens: 1024 },
                        );
                        ...
                        try {
                            if (this.voiceTextChannelId) {
                                await this.sendToChannel(this.voiceTextChannelId, `エラーが発生しました: ${(e as Error).message}`);
                            }
                        } catch (sendError) {
    ...
    private pcmToWav(pcm: Buffer, sampleRate: number, channels: number) {
    ...
    private async runBotChat(channelId: string, otherBot: BotInstance, topic: string, turns: number) {
        type TranscriptLine = { speakerId: string; speakerName: string; content: string };
        ...
        let lastLine: TranscriptLine | undefined;
        ...
        for (let i = 0; i < turns; i++) {
            ...
            const line: TranscriptLine = { speakerId, speakerName: speakerDisplay, content: reply };
    ...
    private buildBotChatHistory(transcript: { speakerId: string; speakerName: string; content: string }[]) {
    ...
    public async generateBotChatReply(prompt: string, history: { role: 'user' | 'model'; parts: string }[]) {
        try {
            const systemInstruction = (this.character.systemInstruction || '').trim() || `You are ${this.character.name}. Stay in-character.`;
            const response = await generateResponse(prompt, history, systemInstruction, this.getGenerationParams());
    ...
    public updateCharacter(character: ICharacter) {
        ...
        if (newWackToken && newWackToken !== (this.lastWackToken || oldWackToken)) {
            ...
            if (this.character.id) {
                db.collection('characters').doc(this.character.id).update({
    ...
    private async handleMessage(message: Message) {
        ...
        if (!isDM && !isMentioned && !this.isAutoReply) return;
...
`;
            ...
            try {
                if (imageAttachments.length > 0) {
                    ...
                    response = await generateResponse(parts, history, combinedSystemInstruction, this.getGenerationParams());
                } else {
    ...
    private async reloadConfig(interaction: CommandInteraction) {
        try {
            ...
            const doc = await db.collection('characters').doc(this.character.id).get();
            if (doc.exists) {
                const newChar = doc.data() as ICharacter;
            ...
            try {
                if (interaction.deferred) {
                    ...
                    await interaction.reply({ content: "Failed to reload configuration.", flags: MessageFlags.Ephemeral });
                }
            } catch (e) {}
        }
    }
    ...
    private async fetchMessageHistory(message: Message): Promise<{ role: 'user' | 'model'; parts: string }[]> {
        try {
            ...
            let firestoreHistory = await ConversationService.getHistory(
                this.character.id!,
                message.author.id,
                guildId,
                50 // Context window
    ...
    private async fetchDiscordMessageHistory(message: Message): Promise<{ role: 'user' | 'model'; parts: string }[]> {
        try {
            ...
            Array.from(messages.values()).reverse().forEach(msg => {
                ...
                if (msg.createdTimestamp <= this.lastResetTime) return;

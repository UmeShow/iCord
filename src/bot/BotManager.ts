...
export class BotManager {
  ...
  private startSync() {
    ...
    db.collection('characters').onSnapshot((snapshot) => {
      ...
      snapshot.docChanges().forEach((change) => {
        const characterData = change.doc.data() as ICharacter;
  ...
  private async addBot(character: ICharacter) {
    ...
    if (this.bots.has(character.id!)) return;
  ...
  private removeBot(characterId: string) {
